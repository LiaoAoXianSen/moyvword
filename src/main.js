...
function resolveStripPosition() {
  ...
  return {
    x: Math.max(area.x, Math.min(saved.x, area.x + area.width - 520)),
    y: Math.max(area.y, Math.min(saved.y, area.y + area.height - 30))
  };
}
...
if (hasSingleInstanceLock) {
  ...
  app.whenReady().then(async () => {
    ...
    dialog.showErrorBox('摸鱼单词v57版本无法启动', '学习数据无法读取，已保留数据库与备份文件。请从备份恢复后重试。');
    app.quit();
  });
}
...
ipcMain.handle('window:strip-size', (_event, expanded) => {
  if (!stripWindow) return;
  stripWindow.setSize(520, expanded ? 132 : 34);
});
...
ipcMain.handle('backup:save', async () => {
  const defaultName = `摸鱼单词v57版本备份-${new Date().toISOString().slice(0, 10)}.sqlite`;
